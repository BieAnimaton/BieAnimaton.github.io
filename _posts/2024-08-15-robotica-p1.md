...
- Monitoramento de velocidade  
    Permite reduzir a velocidade de opração de forma proporcional à proximidade de humanos.
	Manuseio direto
...
## Classificação de robôs
	...
	Pelo tipo de controle
		Não adaptativo (predefinido, determinístico) X adaptativo (reativo, mudanças no ambiente, baseado em comportamento)
...
Interações robôs-objetos - Behaviorismo – Rodney Brooks
	...
	Vários níveis plan reagem ao mesmo estímulo sentido (sense) e
	interferem o sinal de saída (act)
...
Interação humano-robô - Contato físico (Neville Hogan)
	Controle de impedância
		...
		vídeo do braço robótico com a batata
...
Um link tem dimensões e características físicas inerentes ao material que o compõe.  
...
Link: cada para que compõe o braço.  
Juntas: conectam os links e definem os movimentos.  
...
## O que é TCP?
É o ponto de alçao do atuador anexado ao robô nas coordenadas do mundo.
...
É a região no espaço tridimensional onde a mão ou a ferramenta de trabalho (manipulador) consegue alcançar. Depende do projeto mecânico do robô.  
Também define a área de trabalho de um operário humano.  

## O que é resolução
É a menor mudança de posição possível que o robô pode realizar ou que seu sistema de controle pode perceber. É determinada pelo projeto do robô e de seu controle.  
Dois tipos:  
...
É aferida e estimada pelo fabricante para um limite de carga e cargas maiores causam deflexões maiores e degradam a acurácia.  

## Explique repitibilidade.
É a capacidade do robô em repetir uma mesma posição no mesmo espaço e é uma medida estática associada a acurácia.
...
Pertencem à classe de robôs manipuladores PPP (prismáticoprismático-prismático) - possuem as 3 primeiras juntas deslizantes (prismáticas)  
	
Gantry (pórtico)  
Similar aos cartesianos.  
Funciona com base em suportes paralelos na lateral.  
Também são da class PPP.  
...
Desvantagens - menor precisão e suporta menor carga.  
...
Vantagem - permite valores elevados de velocidade e de força.  
Desvantagem - custo elevado, vazamento e manutenção.  
