...
Conjunto de regras que podem ser avaliados em uma determinada situação, devolvendo assim uma resposta baseada nela, ou seja, para cada tipo de regra é retornada uma ação.  
...
Um agente racional deve selecionar uma ação que se espera vir a maximizar sua medida de desempenho para  cada sequencia de percepções possível.  
O conceito de sucesso do agente depende de uma medida de desempenho objetiva.  
> quantidade de sujeira aspirada, gasto de energia, gasto de tempo, quantidade de barulho gerado, etc. 
